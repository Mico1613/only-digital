...
const HistoricalDates: React.FC = () => {
  ...
  const activePeriod = historicalPeriods.find((p) => p.id === activePeriodId) || historicalPeriods[0];
  const activeIndex = historicalPeriods.findIndex((p) => p.id === activePeriodId);
        ...
        <div className={styles.buttons}>
          <button onClick={handlePrev} disabled={activeIndex === 0}>
            ...
            <svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.49988 13.0001L2.49988 7.00012L8.49988 1.00012" stroke="#42567A" strokeWidth="2"/>
            </svg>
          </button>
          <button onClick={handleNext} disabled={activeIndex === historicalPeriods.length - 1}>
            {/* Simple Chevron Right SVG */}
            <svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M1.50012 1.00012L7.50012 7.00012L1.50012 13.0001" stroke="#42567A" strokeWidth="2"/>
            </svg>
          </button>
        </div>
      </div>
